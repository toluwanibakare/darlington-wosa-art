...
export function Button({ variant = 'primary', children, className = '', ...props }: ButtonProps) {
  ...
  const variants = {
    ...
    gold: "px-8 py-4 bg-brand-gold text-brand-white border border-brand-gold hover:border-brand-black rounded-[6px] group"
  };
  ...
  return (
    <button className={`${baseStyles} ${variants[variant]} ${className}`} {...props}>
      ...
      {variant === 'gold' && (
        <span className="absolute inset-0 bg-brand-black transform -translate-x-full group-hover:translate-x-0 transition-transform duration-[400ms] ease-[var(--ease-expo-out)] z-0" />
      )}
      <span className="relative z-10 group-hover:text-brand-white transition-colors duration-[400ms] delay-75">{children}</span>
